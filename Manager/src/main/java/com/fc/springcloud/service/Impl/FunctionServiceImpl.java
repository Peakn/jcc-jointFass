...
public class FunctionServiceImpl implements FunctionService {
    ...
    public int uploadFunction( FunctionDo functionDo) {
        if (functionDo.getId() == null) {
            throw new OutOfBusinessException("Function id cannot empty", ResultCode.BAD_REQUEST.getCode());
        }
    ...
    public Page<FunctionDo> listFunctionByPages(FunctionQuery functionQuery, Pageable page) {
        if (page.getPageNumber() <= 0) {
            throw new OutOfBusinessException("Page index must not be less than zero !", ResultCode.BAD_REQUEST.getCode());
        }
        ...
        return new PageImpl<>(list, PageRequest.of(page.getPageNumber() - 1, page.getPageSize()), functionDoPageInfo.getTotal());
    }
    ...
    public ResponseEntity creatFunction( FunctionDto functionDto) {
        ...
        if (exist != null) {
            throw new EntityExistsException("function '" + functionDto.getFunctionName() + "' already exists in service '" + functionDto.getServiceName() + "'");
        }
        //将Base64编码的字符串转成文件
        String filePath = baseDir + functionDto.getFunctionName() + "_code.zip";
        Boolean aBoolean = FileBase64Util.decryptByBase64(functionDto.getCode().getZipFile(), filePath);
